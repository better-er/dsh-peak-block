...
- Node ≥ 22 即可。插件零运行时依赖，无 `package.json` scripts，无需 `npm install`。
...
4. 触发 `ci.yml`：它会在 `main` 的 push 与所有 PR 上自动跑语法 + 冒烟 + 产物齐全校验。
...
- **发布 = 打 tag**：推送形如 `v0.1.0` 的 tag 即自动发版，无需手动改版本号。`release.yml` 自动执行：
...
- 不要在 PR 中提交多余产物，`node_modules`、`*.tsbuildinfo` 等由 `.gitignore` 排除。
